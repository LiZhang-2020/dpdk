		...
		uint16_t more_frags)
{
	...
	if (idx >= RTE_DIM(fp->frags)) {
		...
		if (fp->key.key_len == IPV4_KEYLEN)
			RTE_IP_FRAG_LOG
				(DEBUG, "%s:%d invalid fragmented packet:\n"
				 "ipv4_frag_pkt: %p, key: <%" PRIx64 ", %#x>, "
				 "total_size: %u, frag_size: %u, last_idx: %u\n"
				 "first fragment: ofs: %u, len: %u\n"
				 "last fragment: ofs: %u, len: %u\n\n",
				 __func__, __LINE__, fp, fp->key.src_dst[0],
				 fp->key.id, fp->total_size, fp->frag_size,
				 fp->last_idx,
	...
	if (likely (fp->frag_size < fp->total_size)) {
		return NULL;
	} else if (can_reassemble_ip_frags(fp)) {
		fp->key.locked = 1;
		TAILQ_REMOVE(&tbl->lru, fp, lru);
		return fp;
	}
...
ip_frag_reasemble(struct ip_frag_pkt *fp, struct rte_ip_frag_death_row *dr)
{
	...
	mb = fp->key.key_len == IPV4_KEYLEN ?
	     ipv4_frag_reassemble(fp) : ipv6_frag_reassemble(fp);
	...
	struct ip_frag_pkt **free, struct ip_frag_pkt **stale)
{
	...
	for (i = 0; i != assoc; i++) {
		...
		else if (!p1[i].key.locked && (max_cycles + p1[i].start < tms))
		...
		else if (!p2[i].key.locked && (max_cycles + p2[i].start < tms))
