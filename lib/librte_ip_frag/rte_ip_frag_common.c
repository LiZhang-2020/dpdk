	...
	uint32_t max_entries, uint64_t max_cycles, int socket_id)
{
	...
	if (rte_is_power_of_2(bucket_entries) == 0 ||
		...
		RTE_IP_FRAG_LOG(ERR, "%s: invalid input parameter\n", __func__);
		return NULL;
	}
	...
	RTE_IP_FRAG_LOG
		(DEBUG, "%s: allocated of %zu bytes at socket %d\n",
		 __func__, sz, socket_id);
...
/*
 * enable ip_frag debug logs with EAL parameter:
 *  --log-level=lib.ip_frag:debug
 */
RTE_LOG_REGISTER(ip_frag_logtype, lib.ip_frag, NOTICE);
