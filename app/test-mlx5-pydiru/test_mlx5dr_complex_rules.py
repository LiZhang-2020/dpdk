...
class Mlx5drComplexTrafficTest(PydiruTrafficTestCase):
    ...
    @staticmethod
    def create_two_root_rules(agr_obj, rte_flow_items, at, actions):
        """
        Create two rules on root table with provided rte_flow_items and actions.
        """
        agr_obj.root_mts.append([Mlx5drMacherTemplate(rte_flow_items[0])])
        agr_obj.root_matchers.append(agr_obj.create_matcher(agr_obj.root_table,
                                                            agr_obj.root_mts[-1], at, prio=0))
        agr_obj.root_rules.append(Mlx5drRule(agr_obj.root_matchers[-1], 0, rte_flow_items[0], 0,
                                             actions, len(actions),
                                             Mlx5drRuleAttr(user_data=bytes(8)), agr_obj.dr_ctx))
        agr_obj.root_rules.append(Mlx5drRule(agr_obj.root_matchers[-1], 0, rte_flow_items[1], 0,
                                             actions, len(actions),
                                             Mlx5drRuleAttr(user_data=bytes(8)), agr_obj.dr_ctx))
    ...
    def root_ttl_drops(agr_obj):
        """
        Creates two rules on root table to drop packets with TTL/hop limit 1 or 0.
        """
        eth_ipv4_ttl0 = u.create_eth_ipv4_rte_items(ttl=0)
        eth_ipv4_ttl1 = u.create_eth_ipv4_rte_items(ttl=1)
        eth_ipv6_ttl0 = [u.create_eth_rte_item(eth_type=u.PacketConsts.ETHER_TYPE_IPV6),
                         u.create_ipv6_rte_item(hop_limits=0), RteFlowItemEnd()]
        eth_ipv6_ttl1 = [u.create_eth_rte_item(eth_type=u.PacketConsts.ETHER_TYPE_IPV6),
                         u.create_ipv6_rte_item(hop_limits=1), RteFlowItemEnd()]
        ...
        agr_obj.root_rules = []
        agr_obj.root_mts = []
        agr_obj.root_matchers = []
        Mlx5drComplexTrafficTest.create_two_root_rules(agr_obj, [eth_ipv4_ttl0, eth_ipv4_ttl1],
                                                       drop_temp, [drop_ra])
        Mlx5drComplexTrafficTest.create_two_root_rules(agr_obj, [eth_ipv6_ttl0, eth_ipv6_ttl1],
                                                       drop_temp, [drop_ra])
    ...
    def test_mlx5dr_action_def33_tag_tir(self):
        ...
        Create root RX matcher(p0) eth/ipv6 ttl = 0 or 1 with action drop.
        Create root RX matcher(p1) dmac/ipv4 with action goto non root table 1.
        Create root RX matcher(p1) dmac/ipv6 with action goto non root table 2.
        Create on non root table 1 RX matchers matching on {eth / IPv4 src dst / UDP src dst}
        or {eth / IPv4 src dst / TCP src dst} with actions TAG + TIR.
        Create on non root table 2 RX matchers matching on {eth / IPv6 src} with actions
        set metadata reg C and  goto table 3
        Create on non root table 3 RX matchers matching on {reg C / eth / IPv6 dst / TCP src dst}
        or {reg c / eth / IPv6 dst / UDP src dst} with actions TAG + TIR.
        Send and verify TCP and UDP packets.
        Packets with TTL 0 and 1 should be dropped.
        """
        ipv4_udp_rte_items = u.create_eth_ipv4_l4_rte_items(next_proto=socket.IPPROTO_UDP)
        ipv4_tcp_rte_items = u.create_eth_ipv4_l4_rte_items(next_proto=socket.IPPROTO_TCP)
        dmac = bytes([int(i, 16) for i in u.PacketConsts.DST_MAC.split(':')])
        dmac_ipv4_rte_items = u.create_eth_ipv4_rte_items(dmac=dmac)
        dmac_ipv6_rte_items = [u.create_eth_rte_item(dst=dmac, eth_type=u.PacketConsts.ETHER_TYPE_IPV6),
                               u.create_ipv6_rte_item(), RteFlowItemEnd()]
        ...
        self.server.root_matcher2, self.server.table2, self.server.root_dest_tbl_rule2 = \
            self.server.create_root_fwd_rule(dmac_ipv6_rte_items)
        self.root_ttl_drops(self.server)
        # Non root
        matcher_ipv4 = self.create_templates_and_matcher(self.server,
                                                         [ipv4_udp_rte_items, ipv4_tcp_rte_items],
        ...
        self.rules = self.create_rules(self.server, matcher_ipv4,
                                       [ipv4_udp_rte_items, ipv4_tcp_rte_items],
                                       [[tag_ra1, tir_ra], [tag_ra2, tir_ra]])
        self.ipv6_src_dst_two_table_rules(self.server, self.server.table2,
                                          me.MLX5DR_TABLE_TYPE_NIC_RX,
                                          [Mlx5drActionTemplate(actions)], [tag_ra1, tir_ra],
                                          [tag_ra2, tir_ra], me.MLX5DR_ACTION_FLAG_HWS_RX)
        # Traffic
        for l3 in  [u.PacketConsts.IP_V4,  u.PacketConsts.IP_V6]:
            packet_udp = u.gen_packet(self.server.msg_size, l3=l3)
            packet_tcp = u.gen_packet(self.server.msg_size, l3=l3, l4=u.PacketConsts.TCP_PROTO)
            packet_ttl_0 = u.gen_packet(self.server.msg_size, l3=l3, ttl=0)
            packet_ttl_1 = u.gen_packet(self.server.msg_size, l3=l3, l4=u.PacketConsts.TCP_PROTO,
                                        ttl=1)
            u.raw_traffic(self.client, self.server, 1, [packet_udp, packet_ttl_0],
    ...
    def ipv6_src_dst_two_table_rules(self, agr_obj, table, table_type, ipv6_at,
                                     udp_actions, tcp_actions, action_flag):
        """
        Creates rule on the provided table to match on {eth / IPv6 src} with action set metadata
        reg C and forward to the next table with matching on {reg c / IPv6 dst / UDP src, dst} or
        {reg c / IPv6 dst / TCP src, dst} with provided UDP and TCP actions.
        """
        # First table
    ...
    def test_mlx5dr_action_def33_encap_tx(self):
        ...
        self.ipv6_src_dst_two_table_rules(self.client, self.client.table,
                                          ...
                                          [encap_ra], me.MLX5DR_ACTION_FLAG_HWS_TX)
        # Verify traffic
        for l3 in [u.PacketConsts.IP_V4, u.PacketConsts.IP_V6]:
