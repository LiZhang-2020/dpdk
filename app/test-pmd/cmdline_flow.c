...
static const struct token token_list[] = {
	...
	[CONFIG_CT_NUMBER] = {
		.name = "connection_tracking_number",
		.help = "number of connection tracking",
		.next = NEXT(next_config_attr,
			     NEXT_ENTRY(UNSIGNED)),
		.args = ARGS(ARGS_ENTRY(struct buffer,
	...
	[PATTERN_TEMPLATE] = {
		...
		.next = NEXT(next_pt_subcmd, NEXT_ENTRY(PORT_ID)),
		.args = ARGS(ARGS_ENTRY(struct buffer, port)),
		.call = parse_template,
	},
	/* Sub-level commands. */
	...
	[PATTERN_TEMPLATE_CREATE_ID] = {
		...
		.next = NEXT(next_pt_attr,
			     NEXT_ENTRY(PATTERN_TEMPLATE_ID)),
	...
	[ACTIONS_TEMPLATE] = {
		...
		.next = NEXT(next_at_subcmd, NEXT_ENTRY(PORT_ID)),
		.args = ARGS(ARGS_ENTRY(struct buffer, port)),
		.call = parse_template,
	},
	/* Sub-level commands. */
	...
	[ACTIONS_TEMPLATE_CREATE_ID] = {
		...
		.next = NEXT(NEXT_ENTRY(ACTIONS_TEMPLATE_MASK),
			     ...
			     NEXT_ENTRY(ACTIONS_TEMPLATE_ID)),
	...
	[TABLE] = {
		...
		.next = NEXT(next_table_subcmd, NEXT_ENTRY(PORT_ID)),
		.args = ARGS(ARGS_ENTRY(struct buffer, port)),
		.call = parse_table,
	},
	/* Sub-level commands. */
	...
	[TABLE_GROUP] = {
		...
		.next = NEXT(next_table_attr, NEXT_ENTRY(GROUP_ID)),
		.args = ARGS(ARGS_ENTRY(struct buffer,
	...
	[TABLE_PRIORITY] = {
		...
		.next = NEXT(next_table_attr, NEXT_ENTRY(PRIORITY_LEVEL)),
		.args = ARGS(ARGS_ENTRY(struct buffer,
	...
	[QUEUE_INDIRECT_ACTION] = {
		...
		.next = NEXT(next_qia_subcmd, NEXT_ENTRY(QUEUE_ID)),
		.args = ARGS(ARGS_ENTRY(struct buffer, queue)),
		.call = parse_qia,
	},
	/* Queue  arguments. */
	...
	[PUSH] = {
		...
		.next = NEXT(NEXT_ENTRY(PUSH_QUEUE), NEXT_ENTRY(PORT_ID)),
		.args = ARGS(ARGS_ENTRY(struct buffer, port)),
		.call = parse_push,
	},
	/* Sub-level commands. */
	...
	[PULL] = {
		...
		.next = NEXT(NEXT_ENTRY(PULL_QUEUE), NEXT_ENTRY(PORT_ID)),
		.args = ARGS(ARGS_ENTRY(struct buffer, port)),
		.call = parse_pull,
	},
	/* Sub-level commands. */
	...
	[ITEM_GRE_OPTION_CHECKSUM] = {
		...
		.next = NEXT(item_gre_option, NEXT_ENTRY(UNSIGNED),
			     item_param),
	...
	[ITEM_PORT_REPRESENTOR_PORT_ID] = {
		.name = "port_id",
		.help = "ethdev port ID",
		.next = NEXT(item_port_representor, NEXT_ENTRY(UNSIGNED),
			     item_param),
	...
	[ACTION_METER_COLOR_MODE] = {
		...
		.next = NEXT(action_meter_mark, NEXT_ENTRY(UNSIGNED)),
		   ...
		   unsigned int ent, char *buf, unsigned int size)
{
	...
	if (ctx->curr == ACTION_MODIFY_FIELD_SRC_TYPE ||
		...
		return rte_strscpy(buf, name, size);
	return -1;
}
...
update_fields(uint8_t *buf, struct rte_flow_item *item, uint16_t next_proto)
{
	...
	case RTE_FLOW_ITEM_TYPE_IPV4:
		...
		if (!ipv4->version_ihl)
			/* Current implementation cannot set 4 IPV4 ihl bits.
			 * If flow rule matches on IPv4::ihl testpmd will assign
			 * both version & ihl bits together.
			 */
