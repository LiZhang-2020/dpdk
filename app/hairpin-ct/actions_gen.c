...
#define MAX_CTX 4000000

static uint32_t nb_ctx[2][RTE_MAX_LCORE];
static struct rte_flow_action_handle *action_ctx[2][16][MAX_CTX];

int
query_ct_object(uint16_t core_id, uint32_t ctx_id, uint16_t port_id, struct rte_flow_error *err)
{
	static struct rte_flow_action_conntrack res_ctx[RTE_MAX_LCORE];
	if (unlikely(!action_ctx[port_id][core_id][ctx_id]))
		return 0;
	return rte_flow_action_handle_query(0, action_ctx[port_id][core_id][ctx_id],
		&res_ctx[core_id], err);
}
...
{
	static const struct rte_flow_modify_conntrack modify_ct_orig = {
			.new_ct = {
					.is_original_dir = 1,
			},
			.direction = 1,
	...
	uint32_t i = 0;
	for (i = 0; i < nb_ctx[port_id][core_id]; i++) {
		if (reset_dir)
			rte_flow_action_handle_update(0, action_ctx[port_id][core_id][i],
							&modify_ct_orig, NULL);
		if (!overwrite)
			rte_flow_action_handle_destroy(0, action_ctx[port_id][core_id][i],
				NULL);
	}
	nb_ctx[port_id][core_id] = 1;
}
...
add_jump(struct rte_flow_action *actions,
	...
	struct additional_para para)
{
	static struct rte_flow_action_jump jump_action[RTE_MAX_LCORE] __rte_cache_aligned;

	do {
		jump_action[para.core_idx].group = para.next_table;
	} while (0);

	actions[actions_counter].type = RTE_FLOW_ACTION_TYPE_JUMP;
	actions[actions_counter].conf = &jump_action[para.core_idx];
}
...
static void
add_vlan_pop(struct rte_flow_action *actions,
	uint8_t actions_counter,
	__rte_unused struct additional_para para)
{
	actions[actions_counter].type = RTE_FLOW_ACTION_TYPE_OF_POP_VLAN;
}
...
extern bool ct_created;

static void
add_ct(struct rte_flow_action *actions,
	uint8_t actions_counter,
	struct additional_para para)
{
	uint16_t port_id = para.port_id;
	static struct rte_flow_action_conntrack ct_conf[RTE_MAX_LCORE] = {
		[0 ... (RTE_MAX_LCORE - 1)] = {
			.peer_port = 0,
			.is_original_dir = 1,
			.enable = 1,
			.live_connection = CT_LIVE_CONNECTION,
			.selective_ack = CT_SELECTIVE_ACK,
			.challenge_ack_passed = CT_CHALLENGE_ACK_PASSED,
			.last_direction = CT_LAST_DIRECTION,
			.liberal_mode = CT_LIBERAL_MODE,
			.state = CT_STATE,
			.max_ack_window = CT_MAX_ACK_WINDOW,
			.retransmission_limit = CT_RETRANSMITION_LIMIT,
			.original_dir = (struct rte_flow_tcp_dir_param) {
				.scale = CT_ORIG_SCALE,
				.close_initiated = CT_ORIG_CLOSE_INITIATED,
				.last_ack_seen = CT_ORIG_LAST_ACK_SEEN,
				.data_unacked = CT_ORIG_DATA_UNACKED,
				.sent_end = CT_ORIG_SENT_END,
				.reply_end = CT_ORIG_REPLY_END,
				.max_win = CT_ORIG_MAX_WIN,
				.max_ack = CT_ORIG_MAX_ACK,
			},
			.reply_dir = (struct rte_flow_tcp_dir_param) {
				.scale = CT_REPLY_SCALE,
				.close_initiated = CT_REPLY_CLOSE_INITIATED,
				.last_ack_seen = CT_REPLY_LAST_ACK_SEEN,
				.data_unacked = CT_REPLY_DATA_UNACKED,
				.sent_end = CT_REPLY_SENT_END,
				.reply_end = CT_REPLY_REPLY_END,
				.max_win = CT_REPLY_MAX_WIN,
				.max_ack = CT_REPLY_MAX_ACK,
			},
			.last_window = CT_LAST_WINDOW,
			.last_index = CT_LAST_INDEX,
			.last_seq = CT_LAST_SEQ,
			.last_ack = CT_LAST_ACK,
			.last_end = CT_LAST_END,
		},
	};
	/* static struct rte_flow_action_handle *action_ctx[RTE_MAX_LCORE]; */
	static const struct rte_flow_indir_action_conf action_ctx_conf = {
		.ingress = 1,
	};
	static const struct rte_flow_modify_conntrack modify_ct_reply = {
		.new_ct = {
			.is_original_dir = 0,
		},
		.direction = 1,
	};
	static struct rte_flow_modify_conntrack modify_ct_orig[RTE_MAX_LCORE] = {
			[0 ... (RTE_MAX_LCORE - 1)] = {
			.new_ct = {
				.is_original_dir = 1,
			},
	...
	static struct rte_flow_action ct_action[RTE_MAX_LCORE] = {
		[0 ... (RTE_MAX_LCORE - 1)] = {
			.type = RTE_FLOW_ACTION_TYPE_CONNTRACK,
			.conf = NULL,
		},
	};
	int ctx_idx = para.reply_dir ? nb_ctx[port_id][para.core_idx] - 1 :
			nb_ctx[port_id][para.core_idx];

	ct_action[para.core_idx].conf = &ct_conf[para.core_idx];

	if (!para.reply_dir) {
		ct_conf[para.core_idx].peer_port = para.port_id;
		if (para.cross_port)
			ct_conf[para.core_idx].peer_port ^= 1;

		if (!ct_created) {
			action_ctx[port_id][para.core_idx][ctx_idx] = rte_flow_action_handle_create
				(para.port_id, &action_ctx_conf,
				&ct_action[para.core_idx], NULL);
		} else {
			modify_ct_orig[para.core_idx].new_ct = ct_conf[para.core_idx];
			rte_flow_action_handle_update(para.port_id,
				action_ctx[port_id][para.core_idx][ctx_idx],
				&modify_ct_orig[para.core_idx], NULL);
		}

		nb_ctx[port_id][para.core_idx]++;
	} else {
		rte_flow_action_handle_update(para.port_id ^ 1,
			action_ctx[port_id][para.core_idx][ctx_idx], &modify_ct_reply, NULL);
	}

	actions[actions_counter].type = RTE_FLOW_ACTION_TYPE_INDIRECT;
	actions[actions_counter].conf = action_ctx[port_id][para.core_idx][ctx_idx];
}

static void
add_ct_route_id(struct rte_flow_action *actions,
	uint8_t actions_counter,
	struct additional_para para)
{
	uint32_t tag = META_DATA + (para.counter % para.ct_route_id_count);
	static struct rte_flow_action_set_tag tag_action[RTE_MAX_LCORE] = {
		[0 ... (RTE_MAX_LCORE - 1)] = {
			.data = RTE_BE32(META_DATA),
			.mask = RTE_BE32(0xffffffff),
			.index = TAG_INDEX,
	...
	tag_action[para.core_idx].index = TAG_INDEX;
	tag_action[para.core_idx].data = RTE_BE32(tag);

	if (!para.cross_port && para.reply_dir)
		tag_action[para.core_idx].index = TAG_INDEX_REPLY;

	actions[actions_counter].type = RTE_FLOW_ACTION_TYPE_SET_TAG;
	actions[actions_counter].conf = &tag_action[para.core_idx];
}

static void
add_aging_shared(struct rte_flow_action *actions,
	uint8_t actions_counter,
	struct additional_para para)
{
	static const struct rte_flow_action_age age_conf = {
		.timeout = AGING_SHARED_TIMEOUT,
	};
	static const struct rte_flow_action age_action = {
		.type = RTE_FLOW_ACTION_TYPE_AGE,
		.conf = &age_conf,
	};
	static const struct rte_flow_indir_action_conf shared_action_conf = {
		.ingress = 1,
	};
	static struct rte_flow_action_handle *shared_action[RTE_MAX_LCORE];


	if (!para.reply_dir)
		shared_action[para.core_idx] = rte_flow_action_handle_create
			(para.port_id, &shared_action_conf, &age_action, NULL);

	actions[actions_counter].type = RTE_FLOW_ACTION_TYPE_INDIRECT;
	actions[actions_counter].conf = shared_action[para.core_idx];

	/* Set shared_action to NULL, next time a new one is created */
	if (para.reply_dir) {
		shared_action[para.core_idx] = NULL;
		if (para.cross_port)
	...
	bool cross_port, bool reply_dir)
{
	...
	additional_para_data = (struct additional_para){
		...
		.port_id = port_id,
		.ct_route_id_count = ct_route_id_count,
	...
	static const struct actions_dict {
		...
		{
			.mask = CT_ROUTE_ID_ACTION,
			.funct = add_ct_route_id,
		},
		{
			.mask = AGING_SHARED_ACTION,
			.funct = add_aging_shared,
		},
	};
