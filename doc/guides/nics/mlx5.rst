...
some capabilities must be granted to the application::
   ...
   setcap cap_sys_admin,cap_net_admin,cap_net_raw,cap_ipc_lock,cap_sys_rawio+ep <dpdk-app>
...
``cap_sys_rawio``
   When creating flows with group > 0 or with transfer.
   For ICM memory allocation.
...
.. table:: Minimal SW/HW versions for rte_flow offloads
   ...
   +-----------------------+-----------------+-----------------+
   | | VLAN                | | DPDK 21.05    | |               |
   | | ingress and /       | | OFED 5.3-1    | |    N/A        |
   | | of_push_vlan /      | | ConnectX-6 Dx | |               |
   +-----------------------+-----------------+-----------------+
   | | VLAN                | | DPDK 21.05    | |               |
   | | egress and /        | | OFED 5.3-1    | |    N/A        |
