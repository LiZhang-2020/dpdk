...
- ``no_traffic_time`` parameter [int]
  ...
  A nonzero value defines the no traffic time in micro-seconds that moves the
  driver to no traffic mode. In this mode, the polling is stopped, and
  interrupts are configured to notify traffic for the driver.
  Default value is 4000000us.
...
e   first time queue creation. Set it together with queues devarg.
