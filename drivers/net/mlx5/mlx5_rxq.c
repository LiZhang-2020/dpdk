...
mlx5_rx_queue_stop_primary(struct rte_eth_dev *dev, uint16_t idx)
{
	...
	ret = priv->obj_ops.rxq_res_modify(rxq, MLX5_RXQ_MOD_RDY2RST);
	if (ret) {
...
mlx5_rx_queue_start_primary(struct rte_eth_dev *dev, uint16_t idx)
{
	...
	*rxq_data->rq_db = rte_cpu_to_be_32(0);
	rte_io_wmb();
	ret = priv->obj_ops.rxq_res_modify(rxq, MLX5_RXQ_MOD_RST2RDY);
	if (ret) {
...
mlx5_rxq_release(struct rte_eth_dev *dev, uint16_t idx)
{
	...
	if (refcnt > 1) {
		...
		priv->obj_ops.rxq_res_release(rxq);
				    ...
				    struct mlx5_ind_table_obj *ind_tbl)
{
	 ...
	 */
	DEBUG("Port %u cannot modify indirection table %p (refcnt %u > 1).",
