...
mlx5_flow_item_template_create(struct rte_eth_dev *dev,
	...
	for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
		...
		case RTE_FLOW_ACTION_TYPE_METER:
		{
			...
			if (fm && !fm->def_policy) {
				...
				/*
				 * Either G or Y RSS configuration patameter
				 * can be used. The queues information will be
				 * retrieved from the policy but not the
		...
		if (copied_n == *indir_n) {
			return rte_flow_error_set
				(error, EINVAL, RTE_FLOW_ERROR_TYPE_ACTION_NUM,
				 NULL, "too many indirect actions");
		}
		      ...
		      struct rte_flow_error *error)
{
	...
	if (priv->sh->meter_aso_en) {
		...
		/* Either G or Y can still use default policy. */
		if (!fm->def_policy) {
		      ...
		      bool external, struct rte_flow_error *error)
{
	...
	if (!wks)
		return 0;
	wks->cb = NULL;
	flow_idx = flow_list_create(dev, type, attr, items,
	...
	if (wks->cb) {
		...
		if (ret) {
			mlx5_flow_list_destroy(dev, type, flow_idx);
			flow_idx = 0;
			goto end;
		}
	}
...
mlx5_flow_create_devx_sq_miss_flow(struct rte_eth_dev *dev, uint32_t txq)
{
	 ...
	 */
	if (mlx5_flow_list_create(dev, MLX5_FLOW_TYPE_CTL, &attr, pattern,
				  actions, false, &error) == 0)
		return 0;
			    ...
			    struct rte_flow_error *error)
{
	...
	return mlx5_flow_list_create(dev, MLX5_FLOW_TYPE_GEN, &attr, pattern,
				     actions, true, error);
}
		 ...
		 struct rte_flow_error *error)
{
	...
	return (void *)(uintptr_t)mlx5_flow_list_create(dev, MLX5_FLOW_TYPE_GEN,
							attr, items, actions,
							true, error);
}
		       ...
		       uint32_t flow_idx)
{
	...
	RTE_VERIFY(wks != NULL);
	wks->cb = NULL;
	flow_list_destroy(dev, type, flow_idx);
	if (wks->cb)
			       ...
			       struct rte_flow_error *error)
{
	...
	return fops->item_template_create(dev, attr, items, error);
}
...
mlx5_flow_create_counter_stat_mem_mng(struct mlx5_dev_ctx_shared *sh)
{
	...
	ret = mlx5_os_wrapped_mkey_create(sh->cdev, mem, size, &mem_mng->wm);
	if (ret) {
		rte_errno = errno;
		mlx5_free(mem);
		return -rte_errno;
			...
			struct rte_flow_error *error __rte_unused)
{
	...
	flow = mlx5_ipool_get(priv->flows
		[MLX5_FLOW_TYPE_GEN], (uintptr_t)(void *)flow_idx);
	if (!flow)
		return -EINVAL;
#ifdef HAVE_IBV_FLOW_DV_SUPPORT
 ...
 *   0 on success, a negative errno value otherwise and rte_errno is set.
 *
 * @note: wrapper for driver action_destroy op callback.
 */
static int
 ...
 */
int
flow_convert_modify_action(struct rte_flow_item *item,
			   ...
			   uint32_t type, struct rte_flow_error *error)
{
	...
	do {
		...
		mask = flow_fetch_field((const uint8_t *)item->mask +
		...
		if (type == MLX5_MODIFICATION_TYPE_COPY) {
			...
			data = flow_fetch_field((const uint8_t *)item->spec +
						field->offset, field->size);
 ...
 */
int
flow_convert_action_modify_ipv4(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 struct rte_flow_error *error)
{
	...
	item.spec = &ipv4;
	item.mask = &ipv4_mask;
	return flow_convert_modify_action(&item, modify_ipv4, NULL, resource,
 ...
 */
int
flow_convert_action_modify_ipv6(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 struct rte_flow_error *error)
{
	...
	item.spec = &ipv6;
	item.mask = &ipv6_mask;
	return flow_convert_modify_action(&item, modify_ipv6, NULL, resource,
					  MLX5_MODIFICATION_TYPE_SET, error);
}
 ...
 */
int
flow_convert_action_modify_mac(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 struct rte_flow_error *error)
{
	...
	item.spec = &eth;
	item.mask = &eth_mask;
	return flow_convert_modify_action(&item, modify_eth, NULL, resource,
					  MLX5_MODIFICATION_TYPE_SET, error);
}
 ...
 */
int
flow_convert_action_modify_vlan_vid(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 struct rte_flow_error *error)
{
...
int
flow_convert_action_modify_tp(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 ...
			 struct rte_flow_error *error)
{
	...
	return flow_convert_modify_action(&item, field, NULL, resource,
					  MLX5_MODIFICATION_TYPE_SET, error);
}

int
flow_convert_action_modify_ttl(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 uint8_t is_ipv4,
			 struct rte_flow_error *error)
{
	...
	if (!action) {
		ttl_value = 0xff;
		type = MLX5_MODIFICATION_TYPE_ADD;
	} else {
		ttl_value = ((const struct rte_flow_action_set_ttl *)
				(action->conf))->ttl_value;
	...
	if (is_ipv4) {
		memset(&ipv4, 0, sizeof(ipv4));
		memset(&ipv4_mask, 0, sizeof(ipv4_mask));
		ipv4.hdr.time_to_live = ttl_value;
 ...
 */
int
flow_convert_action_modify_tcp_seq(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 struct rte_flow_error *error)
{
	...
	item.spec = &tcp;
	item.mask = &tcp_mask;
	return flow_convert_modify_action(&item, modify_tcp, NULL, resource,
 ...
 */
int
flow_convert_action_modify_tcp_ack(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 struct rte_flow_error *error)
{
	const rte_be32_t *conf = (const rte_be32_t *)(action->conf);
 ...
 */
int
flow_convert_action_modify_ipv4_dscp(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 struct rte_flow_error *error)
{
 ...
 */
int
flow_convert_action_modify_ipv6_dscp(struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 struct rte_flow_error *error)
{
	const struct rte_flow_action_set_dscp *conf =
 ...
 */
enum modify_reg
flow_get_metadata_reg(struct rte_eth_dev *dev,
		      const struct rte_flow_attr *attr,
		      struct rte_flow_error *error)
{
	int reg =
			   ...
			   struct rte_flow_error *error)
{
	...
	case RTE_FLOW_FIELD_META:
		return (flow_get_metadata_reg(dev, attr, error) == REG_C_0) ?
	...
	case RTE_FLOW_FIELD_META:
		{
			...
			int reg = flow_get_metadata_reg(dev, attr, error);
			if (reg < 0)
				return;
			MLX5_ASSERT(reg != REG_NON);
			MLX5_ASSERT((unsigned int)reg < RTE_DIM(reg_to_field));
 ...
 */
int
flow_convert_action_modify_field(struct rte_eth_dev *dev,
			 struct mlx5_flow_dv_modify_hdr_resource *resource,
			 const struct rte_flow_action *action,
			 const struct rte_flow_attr *attr,
			 struct rte_flow_error *error)
