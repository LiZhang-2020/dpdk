			 ...
			 bool tunnel_decap, struct rte_flow_error *error)
{
	...
	return flow_convert_action_modify_tp(resource, action, l4_udp,
					     src_port, error);
}
			 ...
			 bool tunnel_decap, struct rte_flow_error *error)
{
	...
	return flow_convert_action_modify_ttl(resource, action, l3_ipv4, error);
}
			 ...
			 int action_type, struct rte_flow_error *error)
{
	...
	MLX5_ASSERT(l3_ipv4 != 0xFF);
	return flow_convert_action_modify_ttl(resource, NULL, l3_ipv4, error);
}
			 ...
			 struct rte_flow_error *error)
{
	...
	return flow_convert_modify_action(&item, reg_c_x, NULL, resource,
	...
	if (config->dv_xmeta_en != MLX5_XMETA_MODE_LEGACY) {
		if (!mlx5_flow_ext_mreg_supported(dev))
			return rte_flow_error_set(error, ENOTSUP,
					  ...
					  "extended metadata register"
					  " isn't supported");
		reg = flow_get_metadata_reg(dev, attr, error);
		if (reg < 0)
			return reg;
		if (reg == REG_NON)
			return rte_flow_error_set(error, ENOTSUP,
				       ...
				       struct rte_flow_error *error)
{
	...
	if (!attr->transfer)
		return rte_flow_error_set(error, EINVAL,
					  ...
					  "match on represented port is valid only when transfer flag is enabled");
	...
	if (!mask)
		mask = &switch_mask;
	if (mask->port_id != 0xffff)
		return rte_flow_error_set(error, ENOTSUP,
					  RTE_FLOW_ERROR_TYPE_ITEM_MASK,
					  mask, "no support for partial mask on \"id\" field");
	ret = mlx5_flow_item_acceptable
				(item, (const uint8_t *)mask,
		...
		if (action->type == RTE_FLOW_ACTION_TYPE_VXLAN_ENCAP)
			...
			encap_data =
				((const struct rte_flow_action_nvgre_encap *)
						action->conf)->definition;
		if (flow_convert_encap_data(encap_data, res.buf,
					    &res.size, error))
			return -rte_errno;
	}
				   ...
				   struct rte_flow_error *error)
{
	...
	if (action_modify_field->dst.field == RTE_FLOW_FIELD_META ||
		...
		ret = flow_get_metadata_reg(dev, attr, error);
	...
	if (action_modify_field->operation == RTE_FLOW_MODIFY_SUB)
		return rte_flow_error_set(error, ENOTSUP,
				RTE_FLOW_ERROR_TYPE_ACTION, action,
				"sub operations is not supported");
	}
	if (action_modify_field->dst.field == RTE_FLOW_FIELD_IPV4_ECN ||
		...
		if (!priv->sh->cdev->config.hca_attr.modify_outer_ip_ecn && root)
			return rte_flow_error_set(error, ENOTSUP,
				RTE_FLOW_ERROR_TYPE_ACTION, action,
				...
				void *match_value)
{
	...
	flow_dv_translate_mlx5_item_tag(dev, match_value,
					&tag_item, MLX5_SET_MATCHER_SW_V);
	flow_dv_translate_mlx5_item_tag(dev, match_mask,
					&tag_item, MLX5_SET_MATCHER_SW_M);
}
...
flow_dv_sft_create_cb(void *tool_ctx __rte_unused, void *cb_ctx)
{
	...
	sft_data->idx = idx;
	sft_data->jump_group = *(uint64_t *)(ctx->data);
	sft_data->port_id = *(uint16_t *)(ctx->data2);
	/* Create an SFT flow */
 ...
 */
int
flow_dv_sft_match_cb(void *tool_ctx __rte_unused,
		     struct mlx5_list_entry *entry, void *cb_ctx)
{
	struct mlx5_flow_cb_ctx *ctx = cb_ctx;
	struct mlx5_flow_sft_data_entry *sft_data =
	...
	return !(*(uint64_t *)(ctx->data) == sft_data->jump_group &&
		 *(uint16_t *)(ctx->data2) == sft_data->port_id);
}

void
flow_dv_sft_remove_cb(void *tool_ctx, struct mlx5_list_entry *entry)
{
...
struct mlx5_list_entry *
flow_dv_sft_clone_cb(void *tool_ctx, struct mlx5_list_entry *oentry,
		     void *cb_ctx)
{
	struct mlx5_flow_cb_ctx *ctx = cb_ctx;
	struct rte_eth_dev *dev = ctx->dev;
	...
	uint32_t idx = 0;
	RTE_SET_USED(tool_ctx);

	sft_data = mlx5_ipool_malloc(sh->ipool[MLX5_IPOOL_SFT], &idx);
	if (!sft_data) {
		rte_flow_error_set(error, ENOMEM,
				   RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
				   NULL,
	...
	memcpy(sft_data, oentry, sizeof(*sft_data));
	sft_data->idx = idx;
	return &sft_data->entry;
}

void
flow_dv_sft_clone_free_cb(void *tool_ctx, struct mlx5_list_entry *entry)
{
	struct rte_eth_dev *dev = tool_ctx;
	struct mlx5_dev_ctx_shared *sh = MLX5_SH(dev);
	struct mlx5_flow_sft_data_entry *sft_data =
			 ...
			 struct rte_flow_error *error)
{
	...
	uint64_t jump_group = sft_key.jump_group;
	uint16_t port_id = (uint16_t)sft_key.port_id;
	...
	flow_sfts = flow_dv_hlist_prepare(priv->sh, &priv->sh->flow_sfts,
				"sfts_table",
				...
				flow_dv_sft_clone_cb,
				flow_dv_sft_clone_free_cb,
				error);
	if (unlikely(!flow_sfts))
		return NULL;
	entry = mlx5_hlist_register(flow_sfts, sft_key.val, &ctx);
	if (!entry) {
		rte_flow_error_set(error, ENOMEM,
				   RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
				   "cannot get sft");
...
flow_dv_matcher_create_cb(void *tool_ctx, void *cb_ctx)
{
	...
	*resource = *ref;
	dv_attr.match_criteria_enable = flow_dv_matcher_enable
							   (resource->mask.buf);
...
flow_dv_dest_array_create_cb(void *tool_ctx __rte_unused, void *cb_ctx)
{
	...
	*resource = *ctx_resource;
	...
	resource->action = mlx5_glue->dr_create_flow_action_dest_array
						(domain,
...
flow_dv_aso_ct_release(struct rte_eth_dev *dev, uint32_t own_idx,
		       struct rte_flow_error *error)
{
	uint16_t owner = (uint16_t)MLX5_ACTION_CTX_CT_GET_OWNER(own_idx);
	uint32_t idx = MLX5_ACTION_CTX_CT_GET_IDX(own_idx);
			    ...
			    struct rte_flow_error *error)
{
	...
	for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
		...
		case RTE_FLOW_ITEM_TYPE_SFT:
			...
			if (ret)
				return -rte_errno;
			wks.last_item = MLX5_FLOW_LAYER_SFT;
			break;
		case RTE_FLOW_ITEM_TYPE_CONNTRACK:
		  ...
		  struct rte_flow_error *error)
{
	...
	for (; !actions_end ; actions++) {
		...
		case RTE_FLOW_ACTION_TYPE_OF_SET_VLAN_VID:
			...
			if (flow_convert_action_modify_vlan_vid
		...
		case RTE_FLOW_ACTION_TYPE_METER:
			...
			break;
		case RTE_FLOW_ACTION_TYPE_SET_IPV4_DSCP:
			if (flow_convert_action_modify_ipv4_dscp(mhdr_res,
							      actions, error))
				return -rte_errno;
			action_flags |= MLX5_FLOW_ACTION_SET_IPV4_DSCP;
			break;
		case RTE_FLOW_ACTION_TYPE_SET_IPV6_DSCP:
			if (flow_convert_action_modify_ipv6_dscp(mhdr_res,
							      actions, error))
				return -rte_errno;
			action_flags |= MLX5_FLOW_ACTION_SET_IPV6_DSCP;
			break;
	...
	ret = flow_dv_translate_items_sws(dev, attr, items, dev_flow,
					  &matcher, error);
	if (ret)
		return -rte_errno;
	if (action_flags & MLX5_FLOW_ACTION_RSS)
			...
			struct rte_mtr_error *error)
{
	...
	*domain_bitmap = def_domain;
	 ...
	 * Only when both G and Y are default, will the default policy
	 * actions be created.
	 */
