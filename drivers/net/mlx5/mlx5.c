...
mlx5_dev_args_check_handler(const char *key, const char *val, void *opaque)
{
	...
	if (strcmp(MLX5_TX_PP, key) == 0) {
		...
		DRV_LOG(WARNING, "\"mtr_log_max_num\" "
			"parameter is not needed anymore for meter feature.");
	} else if (strcmp(MLX5_LOG_MAX_FLOW_PER_MTR, key) == 0) {
		DRV_LOG(WARNING, "\"log_max_flow_per_mtr\" "
			"parameter is not needed anymore for meter feature.");
				...
				struct mlx5_sh_config *config)
{
	...
	DRV_LOG(DEBUG, "\"dv_validate_mod\" is %u.", config->dv_validate_mod);
...
mlx5_lwm_setup(struct mlx5_priv *priv)
{
	...
	priv->sh->devx_channel_lwm = mlx5_glue->devx_create_event_channel
			(priv->sh->cdev->ctx,
...
{
	mlx5_os_interrupt_handler_unset(&sh->intr_handle_lwm,
...
const struct eth_dev_ops mlx5_dev_ops = {
	...
	.rx_queue_release_ex = mlx5_rx_queue_release,
		      ...
		      struct mlx5_port_config *config)
{
	...
	DRV_LOG(DEBUG, "\"isolated_mode\" is %u.", config->isolated_mode);
	DRV_LOG(DEBUG, "\"sft_en\" is %u.", config->sft_en);
	DRV_LOG(DEBUG, "\"sft_dbg\" is %u.", config->sft_dbg);
			       ...
			       struct mlx5_kvargs_ctrl *mkvlist)
{
	...
	if (sh->config.dv_validate_mod ^ config->dv_validate_mod) {
		DRV_LOG(ERR, "\"dv_validate_mod\" "
			"configuration mismatch for shared %s context.",
			sh->ibdev_name);
		goto error;
	}
