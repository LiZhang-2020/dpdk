 ...
 */
static int
mlx5_rxq_modify_wq_vlan_strip(struct mlx5_rxq_priv *rxq, int on)
{
 ...
 */
static int
mlx5_rxq_ibv_res_new(struct mlx5_rxq_priv *rxq)
{
...
/**
 * Release an Rx verbs queue resources.
 *
 * @param rxq
 *   Pointer to Rx queue.
 */
static void
mlx5_rxq_ibv_res_release(struct mlx5_rxq_priv *rxq)
{
...
mlx5_rxq_ibv_obj_drop_create(struct rte_eth_dev *dev)
{
	...
	rxq_ctrl = mlx5_malloc(MLX5_MEM_ZERO, sizeof(*rxq_ctrl), 0,
			       SOCKET_ID_ANY);
	if (rxq_ctrl == NULL) {
		DEBUG("Port %u cannot allocate drop Rx queue control memory.",
		      dev->data->port_id);
		rte_errno = ENOMEM;
		goto error;
	}
...
mlx5_rxq_ibv_obj_dummy_lb_create(struct rte_eth_dev *dev)
{
	...
	if (!sh->self_lb.ibv_cq) {
		DEBUG("Port %u cannot allocate CQ for loopback.",
		      dev->data->port_id);
		rte_errno = errno;
		goto error;
	}
	sh->self_lb.qp = mlx5_glue->dv_create_qp(ctx,
				...
				&qp_init_attr);
	if (!sh->self_lb.qp) {
		DEBUG("Port %u cannot allocate QP for loopback.",
		      dev->data->port_id);
		rte_errno = errno;
		goto error;
	}
...
struct mlx5_obj_ops ibv_obj_ops = {
	.rxq_modify_vlan_strip = mlx5_rxq_modify_wq_vlan_strip,
	.rxq_res_new = mlx5_rxq_ibv_res_new,
	.rxq_event_get = mlx5_rx_ibv_get_event,
	.rxq_res_modify = mlx5_ibv_modify_wq,
	.rxq_res_release = mlx5_rxq_ibv_res_release,
