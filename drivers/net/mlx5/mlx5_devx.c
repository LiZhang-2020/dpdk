...
mlx5_rxq_create_devx_rmp(struct mlx5_rxq_priv *rxq)
{
	...
	rmp = mlx5_devx_cmd_create_rmp(priv->sh->cdev->ctx, &rmp_attr,
				       rxq_ctrl->socket);
	return rmp;
}
...
mlx5_rxq_create_devx_rq_resources(struct mlx5_rxq_priv *rxq)
{
	...
	rxq_data->wqes = buf;
	rxq_ctrl->wq_umem = mlx5_glue->devx_umem_reg(priv->sh->cdev->ctx,
						     buf, wq_size, 0);
	if (!rxq_ctrl->wq_umem) {
	...
	dbr_offset = mlx5_get_dbr(priv->sh->cdev->ctx, &priv->sh->dbrpgs,
				  &dbr_page);
	if (dbr_offset < 0) {
...
mlx5_rxq_obj_hairpin_new(struct mlx5_rxq_priv *rxq)
{
	...
	if (rxq->hairpin_conf.use_locked_device_memory) {
		...
		rxq->devx_rq = mlx5_devx_cmd_create_rq(priv->sh->cdev->ctx, &locked_attr,
						       rxq_ctrl->socket);
		if (!rxq->devx_rq && rxq->hairpin_conf.force_memory) {
			DRV_LOG(ERR, "Port %u Rx hairpin queue %u can't create RQ object"
				     " with locked memory buffer",
				     priv->dev_data->port_id, idx);
			return -rte_errno;
		} else if (!rxq->devx_rq && !rxq->hairpin_conf.force_memory) {
...
create_rq_unlocked:
	rxq->devx_rq = mlx5_devx_cmd_create_rq(priv->sh->cdev->ctx, &unlocked_attr,
					       rxq_ctrl->socket);
	if (!rxq->devx_rq) {
		DRV_LOG(ERR,
