...
struct mlx5_obj_ops {
	...
	int (*rxq_event_get_lwm)(struct mlx5_priv *priv,
				 int *rxq_idx, int *port_id);
...
struct mlx5_priv {
	...
	int32_t representor_id; /* UINT16_MAX if not a representor. */
...
int mlx5_flow_query(const struct rte_eth_dev *dev, uint64_t flow_idx,
